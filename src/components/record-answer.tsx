...
export const RecordAnswer = ({
  ...
  const cleanJsonResponse = (responseText: string) => {
    ...
    // Step 3: Try to extract JSON object from the response
    ...
    try {
      ...
      // Fallback: try to extract just the ratings and feedback manually
  ...
  const generateResult = async (
    ...
    try {
      ...
      // Ensure rating is a valid number between 1-10
      const rating = typeof parsedResult.ratings === 'number' 
        ? Math.max(1, Math.min(10, parsedResult.ratings))
        : 5;
      
      return {
        ratings: rating,
        feedback: parsedResult.feedback || "No feedback available"
      };
    } catch (error) {
      ...
      return { ratings: 5, feedback: "Unable to generate feedback. Please try again." };
    } finally {
  ...
  const saveUserAnswer = async () => {
    ...
    if (!aiResult || !aiResult.feedback || typeof aiResult.ratings !== 'number') {
      toast.error("Error", {
        description: "Invalid feedback data. Please try recording your answer again.",
      });
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!querySnap.empty) {
        ...
        await addDoc(collection(db, "userAnswers"), answerData);
      ...
      // Auto-advance to next question or complete interview with a small delay
      setTimeout(() => {
        if (isLastQuestion && onInterviewComplete) {
          onInterviewComplete();
        } else if (onAnswerSaved) {
          onAnswerSaved();
        }
      }, 1500); // 1.5 second delay to show the saved message
    } catch (error) {
      toast("Error", {
        description: "An error occurred while generating feedback.",
      });
